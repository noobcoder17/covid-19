...
class CurvePainter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Stack(
        children: <Widget>[
          Container(width: size.width,height: size.height,color: Colors.transparent,),
          Transform.rotate(
...
class MyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(int i=0;i<size.width;i++){
      ...
      offsets.add(Offset(i.toDouble(),-y*(size.height/4)+size.height/2));
    }
    Paint paint = Paint()..color=color..strokeWidth=2..style=PaintingStyle.stroke..strokeCap=StrokeCap.round;
